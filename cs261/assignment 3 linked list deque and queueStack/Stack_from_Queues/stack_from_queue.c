/***********************************************************
* Author: Michael Murphy & Kaung Myat Htay Win
* Email: murphym7@oregonstate.edu
...
void listQueueInit(struct Queue* queue) 
{
	...
	struct Link* ml = (struct Link*)malloc(sizeof(struct Link));
	assert(ml != NULL);
	ml->next = NULL;
	queue->head = ml;
	queue->tail = ml;
...
TYPE listQueueRemoveFront(struct Queue* queue) 
{
	...
	TYPE tmpVal = queue->head->next->value;
	struct Link* tmpLink = queue->head->next;
...
{
	/* FIXME: You will write this function */
	assert(stack != NULL && stack->q1 != NULL && stack->q2 != NULL);
	if (stack->q1->head == stack->q1->tail && stack->q2->head == stack->q2->tail){
		return 1;
	}
	return 0;
}
...
{
	/* FIXME: You will write this function */
	assert(stack != NULL && listQueueIsEmpty(stack->q1) == 0);
...
void printQueue(struct Queue* s){
	struct Link* tmpLink = s->head->next;
	while (tmpLink != NULL){
		printf("VAL: %d\n", tmpLink->value);
